...
var mapContainer = document.getElementById('map'), // 지도를 표시할 div 
    mapOption = {
        center: new kakao.maps.LatLng(parseFloat('${place.y}'), parseFloat('${place.x}')), // 지도의 중심좌표
        level: 5 // 지도의 확대 레벨
    };  
...
<table border=1>
<tr>
<td colspan="2">선정된 위치 :  ${place.name}</td>
</tr>
<tr>
<td>x : ${place.x}</td>
<td>y : ${place.y}</td>
...
<c:forEach var="item" items="${ct1placeList}">
	${item.name} - ${item.address} - ${item.phone} - ${item.place_url}<br>
</c:forEach>
